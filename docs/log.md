...
Уточнение юзера: речь про дельта-нейтральную ферму типа RiseX ↔ Pacifica, плюс
посмотреть **Omni (Variational)** и Pacifica. RiseX и Omni в ccxt отсутствуют —
нужны нативные адаптеры; Pacifica в ccxt есть и в скан включена.
...
One-leg скан всех 294 perp-рынков HL за 90д: медиана 7.8% APR, мажоры снова ниже
(BTC 6.6% ранг 177/294, ETH 6.9%, SOL 2.5%). Фильтр consistency ≥ 90% + APR ≥ 15%
прошли 9 монет: CASHCAT 41.3%/99.7%, STBL 27.7%/99.8%, XMR 27.6%/95.2%,
HEMI 26.0%/98.3%, VINE 25.9%/98.5%, PURR 23.6%/97.4%, ZRO 19.3%/96.4%,
APEX 16.2%/98.4%, GRASS 16.2%/100%. CASHCAT/PURR/`XYZ-*` (HIP-3) торгуются
только на HL → второй ноги нет. У остальных ставки сейчас сошлись к базовым
10.95% на всех трёх площадках — то есть это заработок прошлого, не текущий.
